...
} forEach [airlift_cargo_1, airlift_cargo_2, Tonk];
...
/*
Potential additions:
if vehicle can not be airlifted then create helper object that gets slung loaded


_helperObj = createVehicle ["Land_CargoBox_V1_F", [0,0,0]];
_helperObj allowDamage false;
//_helperObj hideObjectGlobal true;
_helperObj disableCollisionWith tonk;

_helperObj attachTo [tonk, getCenterOfMass tonk];
